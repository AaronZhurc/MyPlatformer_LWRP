...
{
    [CreateAssetMenu(fileName = "AITriggerAttack", menuName = "Games/AI/AITriggerAttack")]
    public class AITriggerAttack : StateData
    {
        ...
        public override void UpdateAbility(CharacterState characterState, Animator animator, AnimatorStateInfo stateInfo)
        {
            ...
            if(control.aiProgress.TargetIsDead()){
                control.Attack=false;
                return;
            }
            if(control.Turbo && control.aiProgress.AIDistanceToTarget()<2f){
                FlyingKick(control);
            }else if(!control.Turbo && control.aiProgress.AIDistanceToTarget()<1f){
                ListGroundAttacks[Random.Range(0,ListGroundAttacks.Count)](control);
            }else{
                control.Attack=false;
            }
        ...
        public void FlyingKick(CharacterControl control){
            if(control.aiProgress.TargetIsOnSamePlatform()
               && control.aiProgress.DoFlyingKick){
                control.Attack=true;
            }else{
                control.Attack=false;
            }
